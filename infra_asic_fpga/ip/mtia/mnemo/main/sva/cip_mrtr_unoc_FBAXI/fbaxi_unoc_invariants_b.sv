...
`SV_ASSERT(FVPH_RTR_FV_am_vc_timeout_never_happens, !(&`UNOC_MRTR_N_TX.vc_timeout_cnt));
end
else if(DIR == 1) begin : west_am
`SV_ASSERT(FVPH_RTR_FV_am_vc_timeout_never_happens, !(&`UNOC_MRTR_W_TX.vc_timeout_cnt));
end
else if(DIR == 2) begin : south_am
`SV_ASSERT(FVPH_RTR_FV_am_vc_timeout_never_happens, !(&`UNOC_MRTR_S_TX.vc_timeout_cnt));
end
else if(DIR == 3) begin : east_am
`SV_ASSERT(FVPH_RTR_FV_am_vc_timeout_never_happens, !(&`UNOC_MRTR_E_TX.vc_timeout_cnt));
end
else if(DIR == 4) begin : f0_am
`SV_ASSERT(FVPH_RTR_FV_am_vc_timeout_never_happens, !(&`UNOC_MRTR_FI0_TX.vc_timeout_cnt));
end
else if(DIR == 5) begin : f1_am
`SV_ASSERT(FVPH_RTR_FV_am_vc_timeout_never_happens, !(&`UNOC_MRTR_FI1_TX.vc_timeout_cnt));
end
`endif
