...
/// This file contains credit counters for credit checks 
///////////////////////////////////////////////////

module credit_counter_TX_sva # (

  parameter TX_CREDITS  = 4,
  parameter LEN_WIDTH   = mnm_pkg::MNM_UAXI_ID_LEN_WIDTH
  ...
  localparam CREDIT_WIDTH                           = $clog2(TX_CREDITS+1);
...
//------------------------------------------------------------------------------
    ...
    // TODO: VU: what if credit_counter = 0 and noc_out_credit_release = 0 -> enough_credit ? 
    // Should it be replace as: assign enough_credit        = (credit_counter + noc_out_credit_release > noc_out_len);
    // assign enough_credit        = (credit_counter > noc_out_len) || (credit_counter == noc_out_len && noc_out_credit_release);
    assign enough_credit        = (credit_counter + noc_out_credit_release > noc_out_len);
...
//------------------------------------------------------------------------------
    ...
    /*@ASM Credit release should not be asserted if no traffic has been seen at TX out @*/
    `SV_ASSERT (FVPH_RTR_FV_am_no_credit_release_when_no_pkt         ,    credit_counter == TX_CREDITS[CREDIT_WIDTH-1:0]   |->              !noc_out_credit_release   );

    `ifdef FORMAL
      /*@ASM Credit release should be released eventually @*/
...
//------------------------------------------------------------------------------
    ...
    localparam RX_CREDIT_WIDTH                         = $clog2(RX_CREDITS+1);
    ...
    // TODO: VU: what if credit_counter = 0 and noc_out_credit_release = 0 -> enough_credit ? 
    // Should it be replace as: 
    assign enough_credit        = (credit_counter + noc_in_credit_release > noc_in_len);
    // OLD: assign enough_credit        = (credit_counter > noc_in_len) || (credit_counter == noc_in_len && noc_in_credit_release) ;
...
//------------------------------------------------------------------------------
    ...
    /*@ASM There should be enough credits while making a transaction @*/
