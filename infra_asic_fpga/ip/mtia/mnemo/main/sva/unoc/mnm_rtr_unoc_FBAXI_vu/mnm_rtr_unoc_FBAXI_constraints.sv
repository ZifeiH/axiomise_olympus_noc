///////////////////////////////////////////////////
/// File: mnm_rtr_unoc_FBAXI_constraints.sv
/// This file contains the constrains for unoc FBAXI
///////////////////////////////////////////////////

module mnm_rtr_unoc_fbaxi_constraints # (
  parameter RTR_GRID_COORD_Y_START  = mnm_pkg::MNM_RTR_GRID_COORD_Y_START,
  parameter RTR_GRID_COORD_Y_END    = mnm_pkg::MNM_RTR_GRID_COORD_Y_END,
  parameter RTR_GRID_COORD_X_START  = mnm_pkg::MNM_RTR_GRID_COORD_X_START,
  parameter RTR_GRID_COORD_X_END    = mnm_pkg::MNM_RTR_GRID_COORD_X_END,
  parameter SIDE                    = 0
  ...
  `include "mnm_rtr_unoc_FBAXI_constraints_defines.sv"
  ...
  //?? Why arlen can be >= 1; Since read req can only be 1 beat and no burst ?
  ...
  //?? Why blen can be >= 1; Since write resp can only be 1 beat and no burst ?
  ...
  /*@ASM Legal mnm_id values in tgt_id and org_id for req and resp respectively @*/
  `SV_ASSERT (FVPH_RTR_FV_am_legal_mnm_id_values_for_tgt_id    ,    u_noc_in_valid |-> unoc_in_tgtid_mnm_id inside {0, 1}); 

  /*@ASM Legal mnm_id values in org_id and src_id for req and resp respectively @*/
  `SV_ASSERT (FVPH_RTR_FV_am_legal_mnm_id_values_for_src_id    ,    u_noc_in_valid |-> unoc_in_srcid_mnm_id inside {0, 1}); 
  ...
  if(SIDE != NORTH) begin
    /*@ASM Legal mnmid in orgid for SCW resp  @*/
    `SV_ASSERT (FVPH_RTR_FV_am_legal_mnmid_in_orgid_for_scw_rsp  ,    unoc_in_bvalid && unoc_in_srcid_zcoord[3]? !unoc_in_tgtid_mnm_id : 1);
  end

  if(SIDE != SOUTH && SIDE != NORTH) begin
    /*@ASM Legal mnmid in srcid for SCW resp  @*/
    `SV_ASSERT (FVPH_RTR_FV_am_legal_mnmid_in_srcid_for_scw_rsp  ,    unoc_in_bvalid && unoc_in_srcid_zcoord[3]
